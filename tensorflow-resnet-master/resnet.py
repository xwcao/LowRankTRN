...
BN_EPSILON = 0.001
CONV_WEIGHT_DECAY = 0.0001#0.00004
...
def inference_small(x,
    ...
    return inference_small_config(x, c, trl_type, rank)
...
def inference_small_config(x, c, trl_type, rank):
    ...
    #print(x.get_shape().as_list())# -> 8, 8, 64
    ...
    if (trl_type == "cp"):
        ...
        u1 = tf.get_variable("tcl_gap_{}".format(1), shape=[8,convshape[0]],
            initializer = weight_initializer)
        u2 = tf.get_variable("tcl_gap_{}".format(2), shape=[4,convshape[1]],
            initializer = weight_initializer)
        u3 = tf.get_variable("tcl_gap_{}".format(3), shape=[2,convshape[2]],
            initializer = weight_initializer)
        ...
        # post-net
        #x = tf.reduce_mean(x, reduction_indices=[1, 2], name="avg_pool")
        # print(x.get_shape().as_list()) -> 128, 64
        #x = tf.reshape(x, [10000, -1]) #tf.app.flags.FLAGS.batch_size
        x = tf.reshape(x, [-1, 8*8*64])
    
        if c['num_classes'] != None:
            with tf.variable_scope('fc'):
                x = fc(x, c)
        # print(x.get_shape().as_list()) -> 128, 10
...
def fc(x, c):
    ...
    biases = _get_variable('biases',
                           shape=[num_units_out],
                           initializer=tf.zeros_initializer)
    #print(weights.get_shape().as_list()) #4096, 10
